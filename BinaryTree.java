...
public class BinaryTree {
     ...
     *
     * THERE IS NO NEED TO CHANGE ANY CODE ABOVE. DO NOT FORGET TO PLACE YOUR
     * NAME AND SECTION NUMBER AT THE TOP OF THE FILE.
     *
     * YOU ARE TO WRITE THE METHODS: - replaceValue - findMin - NodesGT -
     * average
     *
     **********************************************************
     */
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        // If the current node's data matches oldVal, replace it
        if (node.data == oldVal) {
            node.data = newVal;
        }
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        int count = (node.data > val) ? 1 : 0; //check if its val is >
    ...
    private int[] averageHelper(Node n) {
        if(n == null) return new int[]{0,0}; //Base Case

        int[] left = averageHelper(n.left);  // [sum, count] from the left subtree
        int[] right = averageHelper(n.right); // [sum, count] from the right subtree

         // Sum the results from left, right, and current node
